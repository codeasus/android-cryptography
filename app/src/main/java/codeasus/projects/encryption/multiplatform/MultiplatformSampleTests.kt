package codeasus.projects.encryption.multiplatform

import android.util.Base64
import android.util.Log
import codeasus.projects.encryption.crypto.AESCryptoUtil
import codeasus.projects.encryption.crypto.RSACryptoUtil
import java.nio.charset.StandardCharsets

object MultiplatformSampleTests {

    private const val CLASS_TAG = "DBG@MPSampleTests"

    fun decryptIOSEncryptedData0() {
        val encryptedIOSData =
            ""
        val decodedData = Base64.decode(encryptedIOSData, Base64.NO_WRAP)
        val decryptedData = RSACryptoUtil.decrypt(decodedData)
        Log.d(CLASS_TAG, "Decrypted Data: ${String(decryptedData, StandardCharsets.UTF_8)}")
    }

    fun decryptIOSEncryptedData1() {
        val encryptedIOSData =
            "0G+t6U3sqnossmGv7pRGG8xpmvh+asIujtzN6HxYyLkKUYCtpsqm8ZUsxM1rsj43a/Y/8/9+iu2mjAJBR4z8hyVfCnefN92Z393+LPWKUMy8+yM5fvN5uwpzbzFKLJB8"
        val encryptedSK =
            ""
        val encryptedIV =
            ""
        val decodedData = Base64.decode(encryptedIOSData, Base64.NO_WRAP)
        val decodedSK = Base64.decode(encryptedSK, Base64.NO_WRAP)
        val decodedIV = Base64.decode(encryptedIV, Base64.NO_WRAP)
        val decryptedSK = RSACryptoUtil.decrypt(decodedSK)
        val decryptedIV = RSACryptoUtil.decrypt(decodedIV)
        val sK = AESCryptoUtil.b64EncodedByteArrayToSK(decryptedSK)
        val iV = AESCryptoUtil.b64EncodedByteArrayToIV(decryptedIV)
        val decryptedData = AESCryptoUtil.decrypt(decodedData, sK, iV)
        Log.d(CLASS_TAG, "Decrypted Data: ${String(decryptedData, StandardCharsets.UTF_8)}")
    }

    fun encryptDataForIOS() {
        val data = "Message: 'əıöğw@#><\",352:?%!)(*?öçş32423🍺🍺🥞🥞😒👌'"
        val iosPK =
            ""
        val pair = AESCryptoUtil.getSKAndIVPair()
        val encryptedData =
            AESCryptoUtil.encrypt(data.toByteArray(StandardCharsets.UTF_8), pair.first, pair.second)
        val b64EncodedEncryptedData = Base64.encodeToString(encryptedData, Base64.NO_WRAP)
        val pK = RSACryptoUtil.b64EncodedStrToPK(iosPK)
        val encryptedSK = RSACryptoUtil.encrypt(pair.first.encoded, pK)
        val encryptedIV = RSACryptoUtil.encrypt(pair.second.iv, pK)
        val b64EncodedEncryptedSK = Base64.encodeToString(encryptedSK, Base64.NO_WRAP)
        val b64EncodedEncryptedIV = Base64.encodeToString(encryptedIV, Base64.NO_WRAP)

        Log.d(CLASS_TAG, "b64EncodedEncryptedData: $b64EncodedEncryptedData;")
        Log.d(CLASS_TAG, "b64EncodedEncryptedSK: $b64EncodedEncryptedSK;")
        Log.d(CLASS_TAG, "b64EncodedEncryptedIV: $b64EncodedEncryptedIV;")
    }

    fun setupRSA() {
        RSACryptoUtil.generateKeyPair()
    }
}